...
import { ViewGenerator, bind, chain, intToString, not, reverse, showInInput, showInSelect, subscribeAutoDispose } from '@lightningkite/rxjs-plus'
import { BehaviorSubject } from 'rxjs'
import { map } from 'rxjs/operators'

//! Declares com.lightningkite.rxexample.vg.ControlsDemoVG
export class ControlsDemoVG implements ViewGenerator, HasTitle {
    ...
    //! Declares com.lightningkite.rxexample.vg.ControlsDemoVG.title
    ...
    public generate(dependency: Window): HTMLElement {
        ...
        this._number.pipe(map((it: number): boolean => (it > 5))).pipe(subscribeAutoDispose(xml.checkAlt, reverse("disabled", not)));
        ...
        this.options.pipe(showInInput(xml.editableAutoText, this.text, )).pipe(showInSelect(xml.spinner, this.text));
